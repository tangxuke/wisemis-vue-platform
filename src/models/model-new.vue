...
export default {
  ...
  methods: {
    ...
    save: function() {
      ...
      this.$http
        .post("http://localhost:3000/model/new", model)
        .then(value => {
          if (value.data.success) {
            ...
            this.$Modal.error({
              title: "系统提示",
              content: "创建架构失败！原因：" + value.data.message
            });
        })
        .catch(err => {
          this.$Modal.error({
            title: "系统提示",
            content: "创建架构失败！原因：" + err.message
          });
        });
    },
    typing: function() {
      if (this.name.endsWith("s")) this.collname = this.name.toLowerCase();
      else this.collname = this.name.toLowerCase() + "s";
      if (this.name.length == 0) this.collname = "";
    }
  }
};
</script>
